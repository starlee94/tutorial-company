...
public class CustomDecoder extends AbstractDecoder implements Decoder {
    ...
    public Object decode(feign.Response response, Type type) throws IOException, DecodeException, FeignException {
        ...
        logger.info("Utilize CustomDecoder to complete response body decryption...");
        ...
        sw.stop();
        logger.info("CustomDecoder decryption time lapsed:{}", sw.getTotalTimeSeconds());
        logger.info("Utilize CustomDecoder to complete response body decryption...: {}", result.substring(0, Math.min(1000, result.length())));
        if (result == null){
            return null;
        }
        JsonObject jsonObject = parseObject(result);
        String code = jsonObject.get("code").getAsString();
        if (GlobalSystemEnum.OK.getRspCode().equals(code)){
